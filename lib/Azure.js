...
module.exports = {
  ...
  publish: function (azureConfig, done) {
    ...
    async.series([this._setToken, this._checkResources, this._getPublishingCredentials, this._uploadContent], function (err, results) {
  ...
  _checkResources: function (callback) {
    ...
    webSiteManagementClient.webSpaces.get(webSpace, function (err, result) {
      ...
      if (err && err.code === 'NotFound') {
        ...
        this._deployResourceGroup(config, function (err, result) {
          if (err) return callback(err);
  ...
  _uploadContent: function (callback) {
    ...
    c.on('ready', function () {
      ...
      this._ftpWalk(tempPublishDir, function (err) {

        if (err) return callback(err);

        async.series([this._createFTPDirectories, this._uploadFTPFiles], function (err, results) {
          c.end();
  ...
  _deployResourceGroup: function (config, done) {
    ...
    this._checkRGExistence(config, function (err, exists, result) {
      if (err) return done(err);

      if (!exists) {
        async.series([this._createResourceGroup, this._deployTemplate, this._checkDeploymentStatus], function (err, results) {
          if (err) return done(err);
          done(null, results);
        });
      } else {
      ...
      list.forEach(function (file) {
        ...
        fs.stat(file, function (err, stat) {
          if (stat && stat.isDirectory()) {
            ...
            this._ftpWalk(file, function (err, res) {
              if (!--pending) done();
            });
          } else {
            ftpFiles.push(file);
